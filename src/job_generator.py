...
class JobGenerator:
    ...
    def _sample_log_error(self, n: int) -> np.ndarray:
        s = self.stats
        mask = self.rng.choice(2, size=n, p=[s.log_error_gmm_w, 1 - s.log_error_gmm_w]) == 0
        a  = (LOG_ERROR_LO  - s.log_error_gmm1_mu) / s.log_error_gmm1_sig
        b  = (LOG_ERROR_CLIP - s.log_error_gmm1_mu) / s.log_error_gmm1_sig
        le = np.empty(n)
        le[mask]  = truncnorm.rvs(a, b, loc=s.log_error_gmm1_mu, scale=s.log_error_gmm1_sig,
                                  size=mask.sum(), random_state=self.rng)
        le[~mask] = self.rng.normal(s.log_error_gmm2_mu, s.log_error_gmm2_sig, (~mask).sum())
        return le
